...
""").lower()
    ...
    def on_initialize(self):
        ...
        #bgr_shape = self.bgr_shape = nn.get4Dshape(input_ch,resolution,resolution)
            ...
            for gpu_id in range(gpu_count):
                with tf.device( f'/{devices[gpu_id].tf_dev_type}:{gpu_id}' if len(devices) != 0 else f'/CPU:0' ):
                    ...
                    if style_transfer_power != 0.0:
                        _,gpu_src_features = self.encoder(tf.stop_gradient(gpu_warped_src),style_transfer_loss=True)
                    ...
                    gpu_src_loss += tf.reduce_mean ( 10*tf.abs( gpu_target_srcm - gpu_pred_src_srcm ),axis=[1,2,3] )

...
""").lower()
        ...
        default_style_transfer = self.options['style_transfer_power'] = self.load_or_def_option('style_transfer_power', 0.0)
        ...
        if self.is_first_run() or ask_override:
            ...
            self.options['style_transfer_power'] = np.clip(io.input_number("Style Transfer power", default_style_transfer, add_info="0.0 .. 1.0",help_message="Style Transfer loss apply"),0.0, 1.0)
    ...
    def on_initialize(self):
        ...
        self.style_transfer_power = style_transfer_power = 0.0 if self.pretrain else self.options['style_transfer_power']
        ...
        with tf.device (models_opt_device):
            if 'df' in archi_type:
                ...
                self.inter_AB = model_archi.Inter(in_ch=encoder_out_ch, ae_ch=ae_dims, ae_out_ch=int(ae_dims/2), name='inter_AB')
                self.inter_B  = model_archi.Inter(in_ch=encoder_out_ch, ae_ch=ae_dims, ae_out_ch=int(ae_dims/2), name='inter_B')
                ...
                self.decoder = model_archi.Decoder(in_ch=2*inter_out_ch, d_ch=d_dims, d_mask_ch=d_mask_dims, name='decoder')
        ...
        if self.is_training:
            ...
            for gpu_id in range(gpu_count):
                with tf.device( f'/{devices[gpu_id].tf_dev_type}:{gpu_id}' if len(devices) != 0 else f'/CPU:0' ):
                    ...
                    gpu_src_loss += tf.reduce_mean ( 10*tf.abs( gpu_target_srcm - gpu_pred_src_srcm ),axis=[1,2,3] ) #mask loss for source
                    ...
                    gpu_dst_loss += tf.reduce_mean(10*tf.square( gpu_target_dstm - gpu_pred_dst_dstm ),axis=[1,2,3] ) # mask loss for target

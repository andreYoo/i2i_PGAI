...
class PGAIModel(ModelBase):
    ...
    def on_initialize_options(self):
        ...
        default_gan_patch_size = self.options['gan_patch_size'] = self.load_or_def_option('gan_patch_size',
                                                                                          self.options[
                                                                                              'resolution'] // 8)
        default_gan_dims = self.options['gan_dims'] = self.load_or_def_option('gan_dims', 16)
        default_style_transfer = self.options['style_transfer_power'] = self.load_or_def_option('style_transfer_power',
                                                                                                0.0)
        ...
        if self.is_first_run() or ask_override:
            ...
            self.options['random_hsv_power'] = np.clip(
                io.input_number("Random hue/saturation/light intensity", default_random_hsv_power,
                                add_info="0.0 .. 0.3",
            ...
            self.options['style_transfer_power'] = np.clip(
                io.input_number("Style Transfer power", default_style_transfer, add_info="0.0 .. 1.0",
                                help_message="Style Transfer loss apply"), 0.0, 1.0)
            ...
            if 'df' in self.options['archi']:
                ...
                self.options['true_face_power'] = 0
        ...
        self.gan_model_changed = (default_gan_patch_size != self.options['gan_patch_size']) or (
                    default_gan_dims != self.options['gan_dims'])
    ...
    def on_initialize(self):
        ...
        if self.is_exporting:
            use_fp16 = io.input_bool("Export quantized?", False,
                                     help_message='Makes the exported model faster. If you have problems, disable this option.')

        self.gan_power = gan_power = 0.0 if self.pretrain else self.options['gan_power']
        self.style_transfer_power = style_transfer_power = 0.0 if self.pretrain else self.options[
            'style_transfer_power']
        ...
        if self.pretrain:
            ...
            self.options_show_override['style_transfer_power'] = 0.0
            ...
            if 'df' in archi_type:
                ...
                if self.is_training:
                    if self.options['true_face_power'] != 0:
                        self.code_discriminator = nn.CodeDiscriminator(ae_dims, code_res=self.inter.get_out_res(),
                                                                       name='dis')
                ...
                pdb.set_trace()
                self.encoder = model_archi.Encoder(in_ch=input_ch, e_ch=e_dims, name='encoder')
                encoder_out_ch = self.encoder.get_out_dims()
                self.inter_AB = model_archi.Inter(in_ch=encoder_out_ch, ae_ch=ae_dims, ae_out_ch=int(ae_dims / 2),
                                                  name='inter_AB')
                self.inter_B = model_archi.Inter(in_ch=encoder_out_ch, ae_ch=ae_dims, ae_out_ch=int(ae_dims / 2),
                                                 name='inter_B')

                # self.inter_AB = model_archi.Inter(in_ch=encoder_out_ch, name='inter_AB')
                # self.inter_B  = model_archi.Inter(in_ch=encoder_out_ch, name='inter_B')

                inter_out_ch = self.inter_AB.get_out_ch()
                # inters_out_ch = inter_out_ch*2
                self.decoder = model_archi.Decoder(in_ch=2 * inter_out_ch, d_ch=d_dims, d_mask_ch=d_mask_dims,
                                                   name='decoder')
                ...
                print(self.inter_AB.summary())
                print(self.inter_B.summary())
                print(self.decoder.summary())

            if self.is_training:
                if gan_power != 0:
                    self.D_src = nn.UNetPatchDiscriminator(patch_size=self.options['gan_patch_size'], in_ch=input_ch,
                                                           base_ch=self.options['gan_dims'], name="D_src")
                ...
                if 'df' in archi_type:
                    ...
                    self.src_only_saveable_weights = self.encoder.get_weights() + self.inter_AB.get_weights() + self.decoder.get_weights()
                    if random_warp:
                        self.src_dst_trainable_weights = self.src_dst_saveable_weights
                    else:
                        self.src_dst_trainable_weights = self.src_dst_saveable_weights #Adv All
                        #self.src_dst_trainable_weights = self.encoder.get_weights() + self.inter_B.get_weights() + self.decoder.get_weights()

                self.src_dst_opt = OptimizerClass(lr=lr, lr_dropout=lr_dropout, lr_cos=lr_cos, clipnorm=clipnorm,
                                                  name='src_dst_opt')
                ...
                if self.options['true_face_power'] != 0:
                    self.D_code_opt = OptimizerClass(lr=lr, lr_dropout=lr_dropout, lr_cos=lr_cos, clipnorm=clipnorm,
                                                     name='D_code_opt')
                ...
                if gan_power != 0:
                    self.D_src_dst_opt = OptimizerClass(lr=lr, lr_dropout=lr_dropout, lr_cos=lr_cos, clipnorm=clipnorm,
                                                        name='GAN_opt')
        ...
        if self.is_training:
            ...
            for gpu_id in range(gpu_count):
                with tf.device(f'/{devices[gpu_id].tf_dev_type}:{gpu_id}' if len(devices) != 0 else f'/CPU:0'):
                    ...
                    # process model tensors = this part is for preparing features to cmopute loss.
                    if 'df' in archi_type:
                        ...
                        gpu_src_code = self.encoder(gpu_warped_src, flatten=False)
                        ...
                        gpu_dst_code = self.encoder(gpu_warped_dst, flatten=False)
                        gpu_dst_inter_B_code = self.inter_B(gpu_dst_code)
                        gpu_dst_inter_AB_code = self.inter_AB(gpu_dst_code)
                        # gpu_dst_code = tf.concat([gpu_dst_inter_B_code,tf.stop_gradient(gpu_dst_inter_AB_code)], nn.conv2d_ch_axis )
                        gpu_dst_code = tf.concat([gpu_dst_inter_B_code, tf.stop_gradient(gpu_dst_inter_AB_code)],
                                                 nn.conv2d_ch_axis)
                        # gpu_dst_code = (gpu_dst_inter_B_code+gpu_dst_inter_AB_code)/2.0
                    ...
                    if style_transfer_power != 0.0:
                        _, gpu_src_features = self.encoder(tf.stop_gradient(gpu_warped_dst), style_transfer_loss=True)
                        _, gpu_src_dst_features = self.encoder(gpu_pred_src_dst, style_transfer_loss=True)
                        # _,gpu_src_dst_features = self.encoder(gpu_pred_dst_dst,style_transfer_loss=True)
                    ...
                    gpu_src_loss += tf.reduce_mean(10 * tf.abs(gpu_target_src_masked_opt - gpu_pred_src_src_masked_opt),
                                                   axis=[1, 2, 3])
                    if eyes_mouth_prio:
                        gpu_src_loss += tf.reduce_mean(
                            300 * tf.abs(gpu_target_src * gpu_target_srcm_em - gpu_pred_src_src * gpu_target_srcm_em),
                            axis=[1, 2, 3])
                    gpu_src_loss += tf.reduce_mean(10 * tf.abs(gpu_target_srcm - gpu_pred_src_srcm),
                                                   axis=[1, 2, 3])  # mask loss for source
                    ...
                    if eyes_mouth_prio:
                        gpu_dst_loss += tf.reduce_mean(
                            300 * tf.abs(gpu_target_dst * gpu_target_dstm_em - gpu_pred_dst_dst * gpu_target_dstm_em),
                            axis=[1, 2, 3])
                    gpu_dst_loss += tf.reduce_mean(10 * tf.square(gpu_target_dstm - gpu_pred_dst_dstm),
                                                   axis=[1, 2, 3])  # mask loss for target
                    ...
                    # latent feature loss
                    # pdb.set_trace()
                    # latent_loss = tf.reduce_mean(tf.square(gpu_dst_inter_AB_code-gpu_src_inter_AB_code))
                    # _ls = gpu_src_inter_AB_code.get_shape()
                    # pos_dist = tf.reduce_sum(tf.square(tf.reshape(gpu_dst_inter_AB_code,[-1,_ls[1]*_ls[2]*_ls[3]])-tf.reshape(gpu_src_inter_AB_code,[-1,_ls[1]*_ls[2]*_ls[3]])),1)
                    # neg_dust = tf.reduce_sum(tf.square(tf.reshape(gpu_dst_inter_AB_code,[-1,_ls[1]*_ls[2]*_ls[3]])-tf.reshape(gpu_dst_inter_B_code,[-1,_ls[1]*_ls[2]*_ls[3]])),1)
                    # latent_triplet_loss = tf.reduce_mean(tf.maximum(pos_dist-neg_dust+self.margin,0.0))

                    gpu_G_loss = gpu_src_loss + gpu_dst_loss  # + 0.1*latent_triplet_loss

                    if style_transfer_power != 0.0:
                        for _src_f, _src_dst_f in zip(gpu_src_features, gpu_src_dst_features):
                            src_gram = nn.gram_matrix(_src_f)
                            dst_gram = nn.gram_matrix(_src_dst_f)
                            _tmp = tf.reduce_mean(tf.square(dst_gram - src_gram))
                            # print(_tmp)
                            gpu_G_loss += style_transfer_power * _tmp
                    ...
                    if gan_power != 0:  # This is the everything of GAN.
                        gpu_pred_src_src_d, \
                        ...
                        if masked_training:
                            ...
                            gpu_G_loss += 0.02 * tf.reduce_mean(
                                tf.square(gpu_pred_src_src_anti_masked - gpu_target_src_anti_masked), axis=[1, 2, 3])
            ...
            with tf.device(nn.tf_default_device_name if len(devices) != 0 else f'/CPU:0'):
                if 'df' in archi_type:
                    ...
                    gpu_dst_code = self.encoder(self.warped_dst, flatten=False)
                    gpu_dst_inter_B_code = self.inter_B(gpu_dst_code)
                    gpu_dst_inter_AB_code = self.inter_AB(gpu_dst_code)
                    gpu_dst_code = tf.concat([gpu_dst_inter_B_code, gpu_dst_inter_AB_code], nn.conv2d_ch_axis)
                    gpu_src_dst_code = tf.concat([gpu_dst_inter_AB_code, gpu_dst_inter_AB_code], nn.conv2d_ch_axis)

                    # gpu_dst_code = nn.to_tensor_format(gpu_dst_code, to_data_format="NCHW",
                    #                                            from_data_format="NHWC")
                    # gpu_src_dst_code = nn.to_tensor_format(gpu_src_dst_code, to_data_format="NCHW",
                    #                                             from_data_format="NHWC")
                    gpu_pred_src_dst, gpu_pred_src_dstm = self.decoder(gpu_src_dst_code)
        ...
        if self.is_training:
            ...
            self.set_training_data_generators([
                SampleGeneratorFace(training_data_src_path, random_ct_samples_path=random_ct_samples_path,
                                    ...
                                    sample_process_options=SampleProcessor.Options(scale_range=[-0.15, 0.15],
                                    ...
                                    output_sample_types=[
                                        {'sample_type': SampleProcessor.SampleType.FACE_IMAGE, 'warp': random_warp,
                                         ...
                                         'ct_mode': ct_mode, 'random_hsv_shift_amount': random_hsv_power,
    ...
    def export_dfm(self):
        ...
        with tf.device(nn.tf_default_device_name):
            ...
            if 'df' in self.archi_type:
                ...
                gpu_dst_code = self.encoder(warped_dst, flatten=False)
                gpu_dst_inter_B_code = self.inter_B(gpu_dst_code)
                gpu_dst_inter_AB_code = self.inter_AB(gpu_dst_code)
                gpu_dst_code = tf.concat([gpu_dst_inter_B_code, gpu_dst_inter_AB_code], nn.conv2d_ch_axis)
                gpu_src_dst_code = tf.concat([gpu_dst_inter_AB_code, gpu_dst_inter_AB_code], nn.conv2d_ch_axis)
    ...
    def onTrainOneIter(self):
        ...
        src_loss, dst_loss = self.src_dst_train(warped_src, target_src, target_srcm, target_srcm_em, warped_dst,
        ...
        if self.gan_power != 0:
            self.D_src_dst_train(warped_src, target_src, target_srcm, target_srcm_em, warped_dst, target_dst,
    ...
    def onGetPreview(self, samples, for_history=False):
        ...
        if self.resolution <= 256:
            ...
            result += [('PGAI', np.concatenate(st, axis=0)), ]
            ...
            result += [('PGAI masked', np.concatenate(st_m, axis=0)), ]
        else:
            result = []
            ...
            for i in range(n_samples):
                ar = S[i], SS[i]
                st.append(np.concatenate(ar, axis=1))
            result += [('PGAI src-src', np.concatenate(st, axis=0)), ]

            st = []
            for i in range(n_samples):
                ar = D[i], DD[i]
                st.append(np.concatenate(ar, axis=1))
            result += [('PGAI dst-dst', np.concatenate(st, axis=0)), ]

            st = []
            for i in range(n_samples):
                ar = D[i], SD[i]
                st.append(np.concatenate(ar, axis=1))
            result += [('PGAI pred', np.concatenate(st, axis=0)), ]

            st_m = []
            for i in range(n_samples):
                ar = S[i] * target_srcm[i], SS[i]
                st_m.append(np.concatenate(ar, axis=1))
            result += [('PGAI masked src-src', np.concatenate(st_m, axis=0)), ]

            st_m = []
            for i in range(n_samples):
                ar = D[i] * target_dstm[i], DD[i] * DDM[i]
                st_m.append(np.concatenate(ar, axis=1))
            result += [('PGAI masked dst-dst', np.concatenate(st_m, axis=0)), ]
            ...
            for i in range(n_samples):
                ...
                st_m.append(np.concatenate(ar, axis=1))
            result += [('PGAI masked pred', np.concatenate(st_m, axis=0)), ]
    ...
    def get_MergerConfig(self):
        ...
        self.options['resolution'], self.options['resolution'], 3), merger.MergerConfigMasked(face_type=self.face_type,

    ...
    def on_initialize(self):
        ...
        with tf.device (models_opt_device):
            if 'df' in archi_type:
                ...
                #self.inter_AB = model_archi.Inter(in_ch=e_dims, ae_ch=ae_dims, ae_out_ch=ae_dims, name='inter_AB')
                #self.inter_B  = model_archi.Inter(in_ch=e_dims, ae_ch=ae_dims, ae_out_ch=ae_dims, name='inter_B')
        ...
        if self.is_training:
            ...
            for gpu_id in range(gpu_count):
                with tf.device( f'/{devices[gpu_id].tf_dev_type}:{gpu_id}' if len(devices) != 0 else f'/CPU:0' ):
                    ...
                    #latent_loss = tf.reduce_mean(tf.square(gpu_src_inter_AB_code-gpu_dst_inter_AB_code)-tf.square(gpu_src_inter_AB_code-gpu_dst_inter_AB_code)+self.margin)

                    gpu_G_loss = gpu_src_loss + gpu_dst_loss + 0.1*latent_loss
